...
const commands = [
  ...
  ...['pivot', 'search', 'identity', 'asset', 'supply-chain', 'malware', 'knowledge'].map(operation => command(`intel.${operation}`, ['intel', operation], 'intel', `intel ${operation} <observable> [--fast|--standard|--full]`, `run normalized ${operation} intelligence`, { auth: 'required', egressClass: 'gateway', capabilities: ['gateway-read'], handler: 'intelligence', outputType: 'record' })),
  ...
  command('osint.greynoise-swarm', ['swarm'], 'osint', 'swarm <search|get|export|unique|timeseries|diff> ...', 'query bounded GreyNoise Project Swarm sessions, pivots, and workspace diffs', { surfaces: WEB, auth: 'required', egressClass: 'gateway', sideEffect: 'browser-download', capabilities: ['gateway-read'], handler: 'swarm', outputType: 'record', completion: { values: ['search', 'get', 'export', 'unique', 'timeseries', 'diff'] } }),
