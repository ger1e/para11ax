...
- Desktop `INITIALIZE` no longer waits for browser audio-unlock settlement: audio activation is still requested synchronously from the user gesture, while the visual boot proceeds even if `AudioContext.resume()` stalls.
